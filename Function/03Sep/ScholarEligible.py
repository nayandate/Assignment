'''
4. Assignment: Scholarship Eligibility System (Using filter(), map(), and sorted() with Lambda Expressions)
...
price = list(map(int,input("Enter the marks: ").split()))
result = sorted(list(map(lambda x:x+5 ,filter(lambda x: x>=75,price))),reverse=True)
